...
//TODO removed INTERNAL
class CacheManager(
    private val storeHandler: StoreHandler = CacheConstant.storeHandler,
    private val timerCacheStore: LruCacheStore? =
        if (CacheConstant.cache.enabled) LruCacheStore.instance else null
) {
    ...
    private fun getMaxAgeFromCacheControl(cacheControl: String?): Long {
        ...
        return try {
            ...
            CacheConstant.cache.maxAge
        }
    }

    private fun isEnabled(): Boolean = CacheConstant.cache.enabled
